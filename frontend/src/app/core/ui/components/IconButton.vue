...
const classes = computed(() => {
  const base =
    'inline-flex items-center justify-center min-h-11 min-w-11 rounded-[10px] transition-colors ' +
  ...
  const variants = {
    ...
    brand: 'text-accent-text hover:bg-accent-soft',
...
<template>
  <button type="button" :class="classes" :data-variant="variant" :aria-label="label" :title="label" :disabled="disabled" @click="emit('click', $event)">
    <slot />
  </button>
</template>
